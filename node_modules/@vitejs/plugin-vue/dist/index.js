...
async function transformMain(code, filename, options, pluginContext, ssr, asCustomElement) {
  ...
  if (devServer && devServer.config.server.hmr !== false && !ssr && !isProduction) {
    ...
    output.push(`import.meta.hot.accept(({ default: ${asCustomElement ? `{ def: updated }` : `updated`}, _rerender_only }) => {`, `  if (_rerender_only) {`, `    __VUE_HMR_RUNTIME__.rerender(updated.__hmrId, updated.render)`, `  } else {`, `    __VUE_HMR_RUNTIME__.reload(updated.__hmrId, updated)`, `  }`, `})`);
  }
  if (ssr) {
  ...
  if (asCustomElement) {
    output.push(`import { defineCustomElement as __ce } from 'vue'`, `export default __ce(_sfc_main)`);
  } else {
